...
async function recalculatePositions(
  ...
  // ── Count total active students in the class ──────────────────────────────
  ...
  // ── Compute OVERALL positions (class-wide rank for every student) ─────────
  // Sort all reports by percentage descending, assign 1, 2, 3...
  ...
  allSorted.forEach((r, i) => {
    overallPositionMap.set(r._id.toString(), i + 1);
  });

  // ── Check if this class needs department-based ranking ────────────────────
  const departments = [
    ...new Set(
      reports.map((r) => {
  ...
  if (!shouldSplitByDept) {
    // ── Primary / JSS — single group, no department split ──────────────────
    // position and overallPosition are identical
    for (let i = 0; i < allSorted.length; i++) {
      await ReportCardModel.findByIdAndUpdate(allSorted[i]._id, {
        position: i + 1,
        overallPosition: i + 1,
        totalStudentsInClass: totalInClass,
        totalStudentsInDept: totalInClass, // same as class count
      });
    }
    return;
  }

  // ── SS classes — split by department ─────────────────────────────────────
  for (const dept of departments) {
    ...
    // Get reports for this department only, sorted by percentage
    const deptReports = reports
      .filter((r) => {
        const snap = r.studentSnapshot as { department?: string };
        return (snap?.department ?? "none") === dept;
      })
      .sort((a, b) => b.percentage - a.percentage);

    // Assign dept position (1, 2, 3...) and carry over overall position
    for (let i = 0; i < deptReports.length; i++) {
      await ReportCardModel.findByIdAndUpdate(deptReports[i]._id, {
        position: i + 1,                                                    // dept rank
        overallPosition: overallPositionMap.get(deptReports[i]._id.toString()) ?? i + 1, // class-wide rank
        totalStudentsInClass: totalInClass,
