...
// ── GET: changed signature from () to (request: NextRequest) ────────────────
export async function GET(request: NextRequest): Promise<NextResponse<ApiResponse<object[]>>> {
  ...
  try {
    ...
    const sessionId = searchParams.get("sessionId"); // ← added

    // If sessionId provided, only return classes that have report cards in that session
    if (sessionId) {
      const distinctClassIds = await ReportCardModel.distinct("class", { session: sessionId });

      if (distinctClassIds.length === 0) {
        return NextResponse.json({ success: true, data: [] });
      }

      const classes = await ClassModel.find({ _id: { $in: distinctClassIds } })
        .populate("subjects", "name code hasPractical")
        .populate("classTeacher", "surname firstName otherName email")
        .sort({ order: 1 })
        .lean();
    ...
    // No sessionId — original behaviour unchanged
    const classes = await ClassModel.find()
      .populate("subjects", "name code hasPractical")
      .populate("classTeacher", "surname firstName otherName email")
      .sort({ order: 1 })
