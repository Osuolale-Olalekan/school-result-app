...
export default function ParentReportsView() {
  ...
  const [selectedReport, setSelectedReport] = useState<IReportCard | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (ref && studentId) {
      ...
      window.history.replaceState(
        {},
        "",
        window.location.pathname + `?studentId=${studentId}`,
      );
    }
  }, [studentId]);
  ...
  const unlockedCount = filteredReports.filter(
    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-gray-500">
          Select a child to view their report cards
        </p>
        <a
          href="/parent/children"
          className="mt-3 inline-block text-amber-600 text-sm hover:underline"
        >
          Go to My Children →
        </a>
      </div>
    ...
    <FileText className="w-5 h-5 text-emerald-600" />
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm text-gray-500 truncate">
                      {report.className} ·{" "}
                      {report.sessionName ?? report.session?.name} Session
                    </p>

                    {/* ── Under review message ── */}
                    {report.underReview && (
                      <p className="text-xs mt-0.5 text-amber-600 font-medium">
                        Result is currently under review by the school
                      </p>
                    )}

                    {/* ── Score info — only when unlocked and not under review ── */}
                    {!report.underReview &&
                      !report.isLocked &&
                      report.percentage !== undefined && (
                        <p className="text-xs text-gray-400 mt-0.5">
                          Score: {report.percentage.toFixed(1)}% · Grade:{" "}
                          {report.grade} · Position: {report.position}/
                      ...
                      <p className="text-xs mt-0.5">
                        {report.reportCardPaid && !report.schoolFeesPaid ? (
                          <span className="text-blue-400">
                            Awaiting school fees confirmation
                          </span>
                        ) : (
                ...
                <div className="shrink-0 min-[380px]:ml-auto">
                  {report.underReview ? (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-600 text-xs font-medium">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 mb-5 space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium text-blue-950">
                        {paymentModal.report.sessionName ??
