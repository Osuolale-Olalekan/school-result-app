...
const nigeriaStates = [
  ...
  { state: "Kwara", lgas: ["Edu: Lafiagi","Ekiti: Araromi-Opin","Ifelodun: Share","Ilorin East: Oke-Oyi","Ilorin South: Fufu","Ilorin West: Oja-Oba","Irepodun: Omu-Aran","Isin: Owu-Isin","Kaiama: Kaiama","Moro: Bode-Saadu","Offa: Offa","Oke-Ero: Ilofa","Oyun: Ilemona","Patigi: Patigi","Asa: Afon","Baruten: Kosubosu"] },
...
// ✅ Student schema — email is OPTIONAL
const studentSchema = z.object({
  surname:   z.string().min(2, "Required"),
  firstName: z.string().min(2, "Required"),
  otherName: z.string().optional(),
  // Email is optional for students — can be blank for young children
  email: z
    .string()
    .optional()
    .refine(
...
const INPUT_CLS =
  "w-full px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-900 bg-white focus:outline-none focus:border-[#1e3a5f]";
...
export default function UserForm({
  ...
  const availableLgas = nigeriaStates.find((s) => s.state === selectedState)?.lgas ?? [];
    ...
    <form
      onSubmit={handleSubmit((data) => {
        ...
        onSubmit({
          ...
          // Send undefined instead of empty string so the API treats it as no email
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input {...register("surname")} className={INPUT_CLS} />
          {typedErrors.surname && <p className="text-red-500 text-xs mt-1">{typedErrors.surname.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">First Name *</label>
          <input {...register("firstName")} className={INPUT_CLS} />
          {typedErrors.firstName && <p className="text-red-500 text-xs mt-1">{typedErrors.firstName.message}</p>}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Other Name</label>
          <input {...register("otherName")} className={INPUT_CLS} />
        </div>
      </div>

      {/* ── Email — optional for students, required for teachers/parents ── */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">
          Email Address {isStudent ? <span className="text-gray-400 font-normal">(optional for young students)</span> : <span className="text-red-500">*</span>}
        </label>
        <input
          {...register("email")}
          type="email"
          placeholder={isStudent ? "Leave blank if student has no email" : ""}
          className={INPUT_CLS}
        />
        {typedErrors.email && <p className="text-red-500 text-xs mt-1">{typedErrors.email.message}</p>}
        {/* Note only for students */}
        {isStudent && (
          <p className="text-xs text-amber-600 mt-1">
            ⚠️ Without an email, the student cannot reset their password — the admin must reset it manually.
          </p>
        )}
      </div>
        ...
        <>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <button type="button" onClick={() => setManualAdmission(!manualAdmission)} className="text-xs text-blue-600 hover:underline">
            ...
            {manualAdmission ? (
              <>
                ...
                />
                <p className="text-xs text-gray-400 mt-1">Must be unique — enter the student&apos;s existing admission number</p>
              </>
            ) : (
              <input value="Will be auto-generated" disabled className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm bg-gray-50 text-gray-400 cursor-not-allowed" />
            )}
          </div>
          ...
          <div>
            ...
            {typedErrors.currentClass && <p className="text-red-500 text-xs mt-1">{typedErrors.currentClass.message}</p>}

            {/* Department — SSS only */}
            {isSSS && (
              <div className="mt-3">
                <label className="block text-sm font-medium text-gray-700 mb-1">Department <span className="text-red-500">*</span></label>
                <select value={department} onChange={(e) => setDepartment(e.target.value as Department)} className={INPUT_CLS}>
                ...
                {isSSS && department === Department.NONE && (
                  <p className="text-red-500 text-xs mt-1">Department is required for SSS students</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 rounded-xl border text-sm text-gray-900 bg-white focus:outline-none ${typedErrors.dateOfBirth ? "border-red-400" : "border-gray-200 focus:border-[#1e3a5f]"}`}
              />
              {typedErrors.dateOfBirth && <p className="text-red-500 text-xs mt-1">{typedErrors.dateOfBirth.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Gender *</label>
              <select {...register("gender")} className={INPUT_CLS}>
                <option value="">Select...</option>
                <option value="male">Male</option>
                <option value="female">Female</option>
              </select>
              {typedErrors.gender && <p className="text-red-500 text-xs mt-1">{typedErrors.gender.message}</p>}
            </div>
          </div>
          ...
          <div>
            ...
            <select {...register("religion")} className={INPUT_CLS}>
              <option value="">Select...</option>
            ...
            {typedErrors.religion && <p className="text-red-500 text-xs mt-1">{typedErrors.religion.message}</p>}
          </div>

          {/* State of Origin */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">State of Origin *</label>
            <select {...register("stateOfOrigin")} onChange={(e) => { register("stateOfOrigin").onChange(e); setSelectedState(e.target.value); }} className={INPUT_CLS}>
              <option value="">Select state...</option>
              {nigeriaStates.map((s) => <option key={s.state} value={s.state}>{s.state}</option>)}
            </select>
            {typedErrors.stateOfOrigin && <p className="text-red-500 text-xs mt-1">{typedErrors.stateOfOrigin.message}</p>}
          </div>

          {/* LGA */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Local Government Area *</label>
            <select {...register("localGovernment")} disabled={!selectedState} className={`${INPUT_CLS} disabled:bg-gray-50 disabled:text-gray-400 disabled:cursor-not-allowed`}>
            ...
            {typedErrors.localGovernment && <p className="text-red-500 text-xs mt-1">{typedErrors.localGovernment.message}</p>}
          </div>

          {/* Guardian & Address */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Guardian Name</label>
            <input {...register("guardianName")} className={INPUT_CLS} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Guardian Phone</label>
            <input {...register("guardianPhone")} className={INPUT_CLS} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Address</label>
            <input {...register("address")} className={INPUT_CLS} />
          </div>
        </>
      )}
      ...
      {selectedRole === UserRole.TEACHER && (
        <>
          <div>
            ...
            <input {...register("qualification")} placeholder="e.g. B.Ed, M.Sc" className={INPUT_CLS} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Specialization</label>
            <input {...register("specialization")} placeholder="e.g. Mathematics, English" className={INPUT_CLS} />
          </div>
        </>
      )}
      ...
      {selectedRole === UserRole.PARENT && (
        <>
          <div>
            ...
            <input {...register("occupation")} className={INPUT_CLS} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Relationship to Child</label>
            <select {...register("relationship")} className={INPUT_CLS}>
              <option value="">Select...</option>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Link Children <span className="text-gray-400 font-normal">(optional)</span>
            </label>
              ...
              {students.length === 0 ? (
                ...
                students.map((s) => {
                  ...
                  return (
                    <div
                      key={s._id}
                      onClick={() => setSelectedChildren((prev) => prev.includes(s._id) ? prev.filter((id) => id !== s._id) : [...prev, s._id])}
      ...
      <div className={`rounded-xl p-3 border ${isStudent ? "bg-amber-50 border-amber-200" : "bg-blue-50 border-blue-200"}`}>
        <p className={`text-xs ${isStudent ? "text-amber-700" : "text-blue-700"}`}>
          {isStudent
            ? "A password will be auto-generated for the student. If an email is provided, it will be sent to them. If not, the admin must share the admission number and password manually."
      ...
      <div className="flex gap-3 pt-2">
        ...
        >
          {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating...</> : "Create User"}
        </button>
      </div>
    </form>
  );
