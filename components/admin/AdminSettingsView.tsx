...
export default function AdminSettingsView() {
  ...
  async function handleSave() {
    ...
    try {
      let signatureUrl = settings.principalSignature;
      let stampUrl = settings.schoolStamp;
  ...
  async function handleRemoveStamp() {
    ...
    try {
      const res = await fetch("/api/admin/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ schoolStamp: null }),
      });
      const json = (await res.json()) as { success: boolean };
      if (json.success) {
        toast.success("Stamp removed");
        setSettings((prev) => ({ ...prev, schoolStamp: undefined }));
        setStampFile(null);
        setStampPreview(null);
      }
    } catch {
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              This signature will automatically appear on all approved report
              cards
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="font-semibold text-gray-900">School Stamp</h2>
            <p className="text-xs text-gray-500">
              This stamp will appear on all approved report cards near the
              signature
            </p>
          </div>
        </div>

        {(settings.schoolStamp ?? stampPreview) && (
          <div className="mb-5 p-4 bg-gray-50 rounded-xl border border-gray-200">
            <p className="text-xs text-gray-500 mb-2 font-medium">
              {stampPreview ? "New stamp (not saved yet):" : "Current stamp:"}
            </p>
            <div className="flex items-center gap-4">
              <img
                src={stampPreview ?? settings.schoolStamp}
                alt="School stamp"
                className="h-16 object-contain bg-white border border-gray-200 rounded-lg p-2"
              />
              {stampPreview ? (
            ...
            {!stampPreview && settings.schoolStamp && (
              <div className="flex items-center gap-1.5 mt-2">
                <CheckCircle className="w-3.5 h-3.5 text-emerald-500" />
                <p className="text-xs text-emerald-600 font-medium">
                  Stamp is active — appearing on all report cards
                </p>
              </div>
            )}
          </div>
        ...
        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-[#1e3a5f] hover:bg-blue-50/30 transition-colors">
          <Upload className="w-6 h-6 text-gray-400 mb-2" />
          <p className="text-sm text-gray-500 font-medium">
            {settings.schoolStamp ? "Upload new stamp" : "Upload stamp image"}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            PNG recommended (transparent background)
          </p>
        ...
        <button
          onClick={handleSave}
          disabled={saving || (!stampFile && !!settings.schoolStamp)}
          className="mt-4 w-full py-2.5 rounded-xl bg-[#1e3a5f] text-white text-sm font-semibold hover:bg-[#152847] disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
        >
          {saving ? (
            <>
        ...
        <div className="space-y-3">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setCleanupType(
                  e.target.value as "audit" | "notifications" | "both",
                )
              }
