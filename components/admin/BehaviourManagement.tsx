...
import {
  ...
  AlertTriangle, ThumbsUp, ThumbsDown, User, Calendar,
} from "lucide-react";
...
const NEGATIVE_CATEGORIES: { value: BehaviourCategory; label: string }[] = [
...
const POSITIVE_CATEGORIES: { value: BehaviourCategory; label: string }[] = [
...
const EMPTY_FORM: BehaviourForm = {
  studentId:   "",
  date:        new Date().toISOString().split("T")[0],
  type:        BehaviourType.NEGATIVE,
  category:    "",
  description: "",
  severity:    "",
  actionTaken: "",
...
export default function BehaviourManagement() {
  ...
  const [form, setForm]                             = useState<BehaviourForm>(EMPTY_FORM);
  ...
  const fetchRecords = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res  = await fetch(`/api/admin/behaviour?${params}`);
  ...
  // ── Search students (debounced 300ms) ──
  const searchStudents = useCallback(async (q: string) => {
    if (q.trim().length < 2) { setStudents([]); return; }
    setIsSearchingStudents(true);
    try {
      const params = new URLSearchParams({ role: "student", status: "active", search: q.trim(), limit: "20", page: "1" });
  ...
  function handleStudentSearchChange(value: string) {
    ...
    studentSearchTimer.current = setTimeout(() => void searchStudents(value), 300);
  }
  ...
  async function handleSave() {
    ...
    setIsSaving(true);
    try {
      const res  = await fetch("/api/admin/behaviour", {
        method:  "POST",
      ...
      toast.success("Behaviour record logged successfully");
      setShowForm(false);
  ...
  const filtered = records.filter((r) => {
    ...
    const name    = student ? `${student.firstName} ${student.surname}`.toLowerCase() : "";
    const adm     = student?.admissionNumber?.toLowerCase() ?? "";
    return name.includes(s) || adm.includes(s) || r.category.includes(s);
  });
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[140px]">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 rounded-xl text-xs outline-none bg-white border border-gray-200 text-gray-900 placeholder-gray-400 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 transition-all"
          />
        </div>
      </div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 line-clamp-2 mb-1.5">{r.description}</p>
                    {r.actionTaken && (
                      <p className="text-[10px] text-gray-400 mb-1.5">Action: {r.actionTaken}</p>
                    )}
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button" onClick={() => handleTypeSwitch(BehaviourType.NEGATIVE)}
                    ...
                    <button
                      type="button" onClick={() => handleTypeSwitch(BehaviourType.POSITIVE)}
                      className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold transition-all border ${
                        form.type === BehaviourType.POSITIVE
                          ? "bg-emerald-50 text-emerald-600 border-emerald-200"
                          : "bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100"
                      }`}
                    >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-8 pr-8 py-2.5 rounded-xl text-sm outline-none transition-all border ${
                        selectedStudent
                  ...
                  {studentSearch.length >= 2 && students.length === 0 && !isSearchingStudents && !selectedStudent && (
