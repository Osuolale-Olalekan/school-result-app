...
const POSITIVE_CATEGORIES = [
  {
    value: BehaviourCategory.OUTSTANDING_PERFORMANCE,
...
> = {
  [BehaviourSeverity.LOW]: {
    label: "Low",
    bg: "bg-amber-50",
    text: "text-amber-600",
    border: "border-amber-200",
  },
  [BehaviourSeverity.MEDIUM]: {
    label: "Medium",
    bg: "bg-orange-50",
    text: "text-orange-600",
    border: "border-orange-200",
  },
  [BehaviourSeverity.HIGH]: {
    label: "High",
    bg: "bg-red-50",
    text: "text-red-600",
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/teacher/behaviour/search-students?q=${encodeURIComponent(value.trim())}`,
        );
        const json = await res.json();
        if (json.success) setFilteredStudents(json.data ?? []);
      } catch {
        // silent — don't toast on search failure
      } finally {
    ...
    try {
      const res = await fetch("/api/teacher/behaviour", {
        ...
        body: JSON.stringify({
          ...
          severity:
            form.type === BehaviourType.NEGATIVE ? form.severity : undefined,
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap mb-1">
                      ...
                      {sevStyle && (
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-semibold border ${sevStyle.bg} ${sevStyle.text} ${sevStyle.border}`}
                        >
                          {sevStyle.label}
                        </span>
                      )}
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[BehaviourType.NEGATIVE, BehaviourType.POSITIVE].map(
                      (t) => (
                        <button
                          key={t}
                          type="button"
                ...
                <div>
                  ...
                  <div className="relative">
                    {/* inside the student search <div className="relative"> */}
                ...
                {form.type === BehaviourType.NEGATIVE && (
                  <div className="flex items-start gap-2 p-3 rounded-xl text-xs bg-red-50 border border-red-100 text-red-600">
                    ...
                    Parent will be automatically notified when this record is
                    saved.
                  </div>
                )}
