...
const SCHOOL = "God's Way Model Schools";
...
// Minimal HTML-escaping for untrusted, visitor-supplied text before it's
// interpolated into an email template.
...
export async function sendContactMessageEmail(
  ...
  const to = process.env.CONTACT_EMAIL_TO ?? "godswaygroupofschools@gmail.com";
  ...
  const html = baseTemplate(`
    <h2 style="color:#1e3a5f;margin:0 0 16px;">New Website Contact Message</h2>
    <p style="color:#374151;line-height:1.6;margin:0 0 4px;"><strong>From:</strong> ${safeName} (${safeEmail})</p>
    <p style="color:#374151;line-height:1.6;margin:0 0 16px;"><strong>Subject:</strong> ${safeSubject}</p>
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px;color:#374151;line-height:1.6;">
      ${safeMessage}
    </div>
    <p style="color:#6b7280;font-size:13px;margin-top:20px;">Reply directly to this email to respond to ${safeName}.</p>
  `);

  await sendEmail({
    to,
    subject: `New Contact Message: ${subject} — ${SCHOOL}`,
    html,
