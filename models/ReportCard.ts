...
export interface IReportCardDocument extends Document {
  ...
  percentage: number;
  position: number;         // department-scoped rank
  overallPosition: number;  // class-wide rank (same as position for Primary/JSS)
...
const ReportCardSchema = new Schema<IReportCardDocument>(
  {
    ...
    position: { type: Number, default: 0 },         // department-scoped rank
    overallPosition: { type: Number, default: 0 },  // class-wide rank
    totalStudentsInClass: { type: Number, default: 0 },
    totalStudentsInDept: { type: Number, default: 0 },
    grade: { type: String, required: true },
